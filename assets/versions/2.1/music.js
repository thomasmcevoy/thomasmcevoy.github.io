...
'use strict';
(function() {
  angular.module('thomas.music').controller('MusicCtrl', function($scope, $http, player) {
    $scope.player = player;
    $http.get('albums.json').success(function(data) {
      $scope.albums = data;
    });
  });
  angular.module('thomas.music').factory('player', function(audio, $rootScope) {
    var player,
        playlist = [],
        paused = false,
        current = {
    ...
    player = {
      ...
      play: function(track, album) {
        ...
        if (angular.isDefined(album))
          current.album = album;
        if (!paused || angular.isDefined(track))
  ...
  angular.module('thomas.music').factory('audio', function($document) {
